"""Annotate a released or collected post CSV with an OpenAI chat model."""
...
def main() -> None:
    ...
    parser.add_argument("--output-csv", type=Path, default=release_dir / "Generated_Output" / "Annotations" / "openai_annotations.csv")
    args = parser.parse_args()
    key = os.environ.get("OPENAI_API_KEY", "").strip()
    if not key:
        raise RuntimeError("Set OPENAI_API_KEY before running annotation.")
    ...
    client, rows = OpenAI(api_key=key), []
